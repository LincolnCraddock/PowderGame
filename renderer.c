...
void r_init(int w, int h) {
  window = (struct fenster) {.title="A window", .width=w, .height=h};
  /* init SDL window */
...
int r_mouse_down(void) {
  if (window.inp.mouse_button_down[0] && !mouse_down) {
    mouse_down = 1;
    return 1;
  }
  return 0;
...
int r_mouse_moved(int *mousex, int *mousey) {
  if ((int)window.inp.mouse_pos[0] != *mousex || (int)window.inp.mouse_pos[1] != *mousey) {
    *mousex = window.inp.mouse_pos[0];
    *mousey = window.inp.mouse_pos[1];
    return 1;
  }
  return 0;
}

int r_ctrl_pressed(void) { return window.inp.key_mod[0]; }

int r_shift_pressed(void) { return window.inp.key_mod[1]; }

int r_alt_pressed(void) { return window.inp.key_mod[2]; }

int r_key_down(int key) {
  if (window.inp.key[key]) {
    return 1;
  }
  return 0;
}
...
struct fenster* r_get_underlying(void) {
  return &window;
}
